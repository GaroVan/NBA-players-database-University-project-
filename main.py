...
def createGUI():
    ...
    rebounds_produced_tab = ttk.Frame(tab_control)
    draft_produced_tab = ttk.Frame(tab_control)
    ...
    tab_control.add(rebounds_produced_tab, text ='Top Rebounders')
    tab_control.add(draft_produced_tab, text ='Best Players by Draft Pick')
    ...
    # Top Rebounders
    labelText = """
    \t Search for the top X players with the most total rebounds
    \n\t X input should be an integer within [0-605]
    \n\t Example: If amount = 25, finds top 25 players with most rebounds
    """
    rebounds_title= ttk.Label(rebounds_produced_tab, text=labelText)
    rebounds_title.pack()
    rebounds_label = ttk.Label(rebounds_produced_tab, text='Enter Amount:')
    rebounds_label.pack()
    rebounds_entry = ttk.Entry(rebounds_produced_tab)
    rebounds_entry.pack()
    rebounds_button = ttk.Button(rebounds_produced_tab, text='Search', command=lambda: getRebounds(rebounds_entry.get(),rebounds_produced_tab))
    rebounds_button.pack()

    # Draft pick
    labelText = """
    \t Search for the best players in terms of points for a given draft pick
    \n\t X input should be an integer within [0-60]
    \n\t Example: If Pick = 1, finds the best players chosen first overall in the draft
    """
    draft_title= ttk.Label(draft_produced_tab, text=labelText)
    draft_title.pack()
    draft_label = ttk.Label(draft_produced_tab, text='Enter Pick:')
    draft_label.pack()
    draft_entry = ttk.Entry(draft_produced_tab)
    draft_entry.pack()
    draft_button = ttk.Button(draft_produced_tab, text='Search', command=lambda: draft_picks(draft_entry.get(),draft_produced_tab))
    draft_button.pack()
...
def top_coaches(topAmt, top_coaches_tab):
    ...
    cursor = dbConnection.cursor()
    try:
        cursor.execute(f"""SELECT TOP (?) fname, lname, wins, losses, teamname, League.league_name FROM Head_Coach
...
def getRebounds(amount, rebounds_produced_tab):
    amount = sanitize_input(amount)
    dbConnection = connect_to_db()
    cursor = dbConnection.cursor()
    try:
        cursor.execute(f"""SELECT TOP (?) fname, lname, teamname, off_rebounds, def_rebounds, off_rebounds + def_rebounds as 'Total Rebounds'
        FROM Player
        JOIN Play_Rebounds ON Player.pid = Play_Rebounds.pid
        ORDER BY 'Total Rebounds' DESC
        """, int(amount))
        result = [{cursor.description[index][0]: column for index, column in enumerate(value)} for value in cursor.fetchall()]
        openNewWindow(pd.DataFrame(result))
    except Exception as inst:
...
def draft_picks(amount, draft_produced_tab):
    amount = sanitize_input(amount)
    dbConnection = connect_to_db()
    cursor = dbConnection.cursor()
    try:
        cursor.execute(f"""SELECT fname, lname, teamname, points, draft_pick FROM Player
        Join Draft_Records ON Draft_Records.pid = Player.pid
        WHERE draft_pick = ? and points <> 0
